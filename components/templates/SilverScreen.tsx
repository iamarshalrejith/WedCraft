...
function FamilySection({ couple }: { couple: CoupleDetails }) {
  ...
  const ParentsCard = ({ sideLabel, fatherName, motherName, delay }: { sideLabel: string; fatherName?: string; motherName?: string; delay: number }) => (
    <Reveal delay={delay}>
      <div style={{ background:"rgba(212,175,55,0.05)", border:"1px solid rgba(212,175,55,0.2)", borderRadius:8, padding:"22px 16px", textAlign:"center", height:"100%" }}>
        ...
        {fatherName && (<div style={{ marginBottom: motherName ? 12 : 0 }}>
          <p style={{ fontFamily:"'Abril Fatface',serif", fontSize:16, color:"#D4AF37", lineHeight:1.3 }}>{fatherName}</p>
          <p style={{ fontSize:9, color:"rgba(212,175,55,0.45)", letterSpacing:"0.15em", textTransform:"uppercase" as const, marginTop:2 }}>Father</p>
        </div>)}
        {fatherName && motherName && <div style={{ height:"0.5px", background:"rgba(212,175,55,0.2)", margin:"10px auto", width:36 }} />}
        {motherName && (<div>
          <p style={{ fontFamily:"'Abril Fatface',serif", fontSize:16, color:"#D4AF37", lineHeight:1.3 }}>{motherName}</p>
  ...
  const RelativeCard = ({ rel, delay }: { rel: { name: string; relation: string; spouseName?: string }; delay: number }) => (
    <Reveal delay={delay}>
      <div style={{ background:"rgba(212,175,55,0.05)", border:"1px solid rgba(212,175,55,0.2)", borderRadius:6, padding:"11px", textAlign:"center", borderLeft:"2px solid rgba(212,175,55,0.2)" }}>
        ...
        <p style={{ fontFamily:"'Abril Fatface',serif", fontSize:14, color:"#D4AF37", lineHeight:1.3 }}>{rel.name}</p>
        {rel.spouseName && (<>
          <div style={{ display:"flex", alignItems:"center", gap:5, justifyContent:"center", margin:"4px 0" }}>
            <div style={{ height:"0.5px", flex:1, background:"rgba(212,175,55,0.2)" }} />
            <span style={{ fontSize:9, color:"rgba(212,175,55,0.45)" }}>&amp;</span>
            <div style={{ height:"0.5px", flex:1, background:"rgba(212,175,55,0.2)" }} />
          </div>
          <p style={{ fontFamily:"'Abril Fatface',serif", fontSize:13, color:"#D4AF37", lineHeight:1.3, opacity:0.7 }}>{rel.spouseName}</p>
        </>)}
      </div>
    </Reveal>
  );
    ...
    <section style={{ padding:"20px 20px 52px", maxWidth:580, margin:"0 auto" }}>
      <Reveal>
        <div style={{ textAlign:"center", marginBottom:26 }}>
          ...
          <p style={{ fontFamily:"'Abril Fatface',serif", fontSize:22, color:"#D4AF37" }}>Our Families</p>
        </div>
      </Reveal>
      {hasParents && (
