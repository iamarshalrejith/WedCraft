  ...
  return (
    <section className="max-w-3xl mx-auto px-6 py-10">
      <div style={{ textAlign:"center", marginBottom:24 }}>
        <p style={{ fontSize:9, letterSpacing:"0.4em", color:"rgba(255,45,120,0.6)", textTransform:"uppercase" as const, marginBottom:6 }}>With Family Blessings</p>
        <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:22, color:"#FF2D78" }}>Our Families</p>
      </div>
      {hasParents && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom: hasRelatives ? 24 : 0 }}>
          {(c.groomFatherName || c.groomMotherName) && (
            <div style={{ background:"rgba(255,45,120,0.06)", border:"1px solid rgba(255,45,120,0.25)", borderRadius:10, padding:"20px 16px", textAlign:"center" }}>
              <p style={{ fontSize:8, letterSpacing:"0.3em", color:"rgba(255,45,120,0.6)", textTransform:"uppercase" as const, marginBottom:12 }}>{c.groomName}&apos;s Parents</p>
              {c.groomFatherName && (<div style={{ marginBottom: c.groomMotherName ? 10 : 0 }}>
                <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:15, color:"#FF2D78", lineHeight:1.3 }}>{c.groomFatherName}</p>
                <p style={{ fontSize:9, color:"rgba(255,45,120,0.6)", textTransform:"uppercase" as const, marginTop:2 }}>Father</p>
              </div>)}
              {c.groomFatherName && c.groomMotherName && <div style={{ height:"0.5px", background:"rgba(255,45,120,0.25)", margin:"8px auto", width:32 }} />}
              {c.groomMotherName && (<div>
                <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:15, color:"#FF2D78", lineHeight:1.3 }}>{c.groomMotherName}</p>
                <p style={{ fontSize:9, color:"rgba(255,45,120,0.6)", textTransform:"uppercase" as const, marginTop:2 }}>Mother</p>
              </div>)}
            </div>
          )}
          {(c.brideFatherName || c.brideMotherName) && (
            <div style={{ background:"rgba(255,45,120,0.06)", border:"1px solid rgba(255,45,120,0.25)", borderRadius:10, padding:"20px 16px", textAlign:"center" }}>
              <p style={{ fontSize:8, letterSpacing:"0.3em", color:"rgba(255,45,120,0.6)", textTransform:"uppercase" as const, marginBottom:12 }}>{c.brideName}&apos;s Parents</p>
              {c.brideFatherName && (<div style={{ marginBottom: c.brideMotherName ? 10 : 0 }}>
                <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:15, color:"#FF2D78", lineHeight:1.3 }}>{c.brideFatherName}</p>
                <p style={{ fontSize:9, color:"rgba(255,45,120,0.6)", textTransform:"uppercase" as const, marginTop:2 }}>Father</p>
              </div>)}
              {c.brideFatherName && c.brideMotherName && <div style={{ height:"0.5px", background:"rgba(255,45,120,0.25)", margin:"8px auto", width:32 }} />}
              {c.brideMotherName && (<div>
                <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:15, color:"#FF2D78", lineHeight:1.3 }}>{c.brideMotherName}</p>
                <p style={{ fontSize:9, color:"rgba(255,45,120,0.6)", textTransform:"uppercase" as const, marginTop:2 }}>Mother</p>
              </div>)}
            </div>
          )}
      ...
      {hasRelatives && (
        <>
          <div style={{ display:"flex", alignItems:"center", gap:10, margin:"4px 0 16px" }}>
            <div style={{ flex:1, height:"0.5px", background:"rgba(255,45,120,0.25)" }} />
            <p style={{ fontSize:8, letterSpacing:"0.28em", color:"rgba(255,45,120,0.6)", textTransform:"uppercase" as const, whiteSpace:"nowrap" as const }}>Extended Family</p>
            <div style={{ flex:1, height:"0.5px", background:"rgba(255,45,120,0.25)" }} />
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {groomRelatives.length > 0 && (<>
                <p style={{ fontSize:7, letterSpacing:"0.22em", color:"rgba(255,45,120,0.6)", textTransform:"uppercase" as const, textAlign:"center", marginBottom:4 }}>{c.groomName}&apos;s Side</p>
                {groomRelatives.map((rel, i) => (
                  <div key={i} style={{ background:"rgba(255,45,120,0.06)", border:"1px solid rgba(255,45,120,0.25)", borderRadius:6, padding:"10px", textAlign:"center", borderLeft:"2px solid rgba(255,45,120,0.25)" }}>
                    <p style={{ fontSize:8, color:"rgba(255,45,120,0.6)", letterSpacing:"0.18em", textTransform:"uppercase" as const, marginBottom:5 }}>{rel.relation}</p>
                    <p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:13, color:"#FF2D78", lineHeight:1.3 }}>{rel.name}</p>
                    {rel.spouseName && (<><p style={{ fontSize:9, color:"rgba(255,45,120,0.6)", margin:"3px 0" }}>&amp;</p><p style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:12, color:"#FF2D78", opacity:0.75 }}>{rel.spouseName}</p></>)}
                  </div>
                ))}
              </>)}
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {brideRelatives.length > 0 && (<>
                <p style={{ fontSize:7, letterSpacing:"0.22em", color:"rgba(255,45,120,0.6)", textTransform:"uppercase" as const, textAlign:"center", marginBottom:4 }}>{c.brideName}&apos;s Side</p>
