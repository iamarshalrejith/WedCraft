...
import {
  ...
  ShieldCheck, LayoutGrid, Eye, TrendingUp,
} from "lucide-react";
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <Link href="/admin/templates/new" className="text-sm font-medium bg-black text-white px-4 py-2 rounded-xl">
                ...
                {templates.map((t) => (
                  <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${tierColors[t.tier]}`}>
                        {t.tier}
                      </span>
                    </td>
                    <td className="px-4 py-4">
                      <span className="text-sm text-gray-600">{t.religion}</span>
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex gap-1.5 flex-wrap">
                        {t.isFeatured && (
                          <span className="text-xs px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full">Featured</span>
                        )}
                        {t.isNew && (
                          <span className="text-xs px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded-full">New</span>
                        )}
                        {!t.isFeatured && !t.isNew && (
                          <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full">Active</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">
